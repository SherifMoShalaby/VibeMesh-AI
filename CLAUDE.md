...
- `src/lib/openscad/worker.ts`: **openscad-wasm is single-shot** — `callMain` may run only once per instance, so a fresh instance is created per render (cheap after first load; the WASM is base64-embedded in a ~14MB JS chunk, hence the raised `chunkSizeWarningLimit`).
