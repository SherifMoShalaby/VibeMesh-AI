...
- Curve resolution is controlled by the app's quality presets (\`$fa\`/\`$fs\`). Do NOT set a global \`$fn\`, and never expose a \`$fn\` / \`segments\` / \`resolution\` parameter. On ordinary round features (holes, bores, fillets, posts, studs) set NO \`$fn\` at all — let the presets drive smoothness, so the quality slider works. Use a per-call \`$fn\` ONLY when the segment count IS the geometry, not its resolution (e.g. \`$fn = 6\` for a hex socket, \`$fn = 3\` for a triangular feature). NEVER put a per-call \`$fn\` on a piece BODY, silhouette, dome, neck, or base tier — that \`$fn\` is FIXED and the quality slider can no longer smooth it, producing a permanent low-poly look. Per-call \`$fn\` is ONLY for a discrete-COUNT feature where the count IS the geometry (hex socket, triangular boss), or a deliberately-faceted ACCENT region clearly SMALLER than and intersected onto an otherwise-smooth body — never the primary mass.
...
When the reference reads HARD-SURFACE / sci-fi / mechanical / armored (smooth curved bodies with crisp recessed panel lines, chamfered edges, layered plates, a few intentional flats, recessed glowing accent lines) — NOT a plain functional part — the goal is a SMOOTH machined surface broken up by CUTS and GREEBLES, never a polygonal low-poly body. BOTH of these fail the reference: (a) a pure axially-symmetric \`rotate_extrude\` (every section a circle — lathe-smooth and featureless), AND (b) an all-over faceted body (a coarse polygonal look FURTHER from the reference than a smooth one). Break the symmetry with CUTS and unioned detail, NOT by polygonizing. Keep every result manifold and self-supporting (new edges <=45deg or chamfered), in priority order:
- SMOOTH BODY FIRST. Build the silhouette from ordinary round primitives, \`hull()\`, or a single \`rotate_extrude\`, with NO per-call \`$fn\` on it, so the quality slider smooths it. NEVER intersect a whole body, dome, neck or base tier with a low-\`$fn\` prism to "panel" it.
...
- An ENERGY-CORE sphere stays a SMOOTH sphere: \`difference()\` out a thin equatorial \`rotate_extrude\` groove (HORIZONTAL, in XY, never tilted), a rotational \`for\`-loop of a few radial seam cutters (width >=0.8mm, n<=8), and a top cylinder recess (depth >=0.8mm) for the polar sensor. Do NOT facet the sphere itself.
...
For a SET of related pieces, drive the base from ONE parameterized module but give EACH piece its OWN arguments (tier count, notch count, diameter) so each base reads as part of THAT piece — never one generic disc-stack reused verbatim. Reserve plain un-broken \`rotate_extrude\` for genuinely smooth turned forms (a vase, a knob).
...
- SCOPE GATE: apply the "Hard-surface / faceted styling" section ONLY when the reference is visibly stylized (sci-fi / armored / greebled / glowing-accent). A PLAIN FUNCTIONAL part (bracket, knob, gear, enclosure, mount, clip, stand) gets ZERO panel lines, ZERO facets, ZERO decorative channels, and only the chamfers PRINTABILITY requires (self-supporting 45° overhangs, exposed-edge breaks) — never chamfers-for-looks; ordinary smooth primitives and revolves only. If unsure, treat it as functional and keep it smooth.
