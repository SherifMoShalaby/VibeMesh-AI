...
- Organic / figurative SILHOUETTE subject (a horse head, fish, animal, leaf — NOT a functional part, which stays smooth primitives): two library-free routes —
  - SIDE-PROFILE EXTRUSION (best for any recognizable silhouette — the subject IS its side profile plus a width): hand-author the outline as \`polygon()\` of ~12-30 points tracing the silhouette, \`linear_extrude(width, center=true)\`, then break the razor edges with a CONCAVITY-PRESERVING bevel — \`minkowski()\` the extrude with a SMALL \`sphere(r=0.6)\` (a single figure slab is simple enough that the Minkowski cost is fine here). NEVER chamfer a NON-CONVEX silhouette with \`hull()\` of two inset extrudes: \`hull()\` returns the CONVEX hull, which FILLS the very concavities the shape needs (the mouth notch, the gap between the ears, the dished cheek) and collapses a horse head into a featureless coffin slab. (\`hull()\` of corner cylinders is still fine for a CONVEX rounded form — just never around a non-convex outline.) Then \`difference()\` in an eye/detail recess + 2-4 panel-line cuts; translate so the lowest point is z=0.
...
5. The parameter block parses under the Customizer rules above and exposes no \`$fn\`/segments/resolution parameter (a \`facets\` parameter IS allowed when the FACET VERDICT is GENUINELY-FACETED, on an image turn).
