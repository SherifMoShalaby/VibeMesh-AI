...
- **Reference-IoU oracle is DEFAULT-ON for image turns (OC-2).** After the adopted render compiles, the silhouette of the adopted STL (best of its fixed render poses, all 8 dihedral photo orientations) is measured against the GrabCut-segmented reference photo via `bestRefIoU` (`src/lib/silhouette.ts` + `src/lib/refSegment.ts`) — pure CPU rasterization that never spends the openscad render budget. No user toggle; independent of the default-OFF best-of-N. The reference mask is read INSIDE the compile path after a fire-and-forget `ensureRefMask`, so a cold-start segmentation race simply yields no IoU that pass (a no-op, identical to the pre-OC-2 path). Below an IoU floor (`REF_IOU_FLOOR ≈ 0.55`) the measured visual-match discrepancy is injected at the head of the next refine prompt (`takeRefineDiscrepancy`), and the loop only continues while a pass keeps RAISING IoU (`iouRefineDecision`) — a non-improving pass stops the chain. The silhouette math is bit-identical to the bench port (`bench/silhouette-live.selftest.mjs`).
- **Proportion-aware shape term (OC-10, `src/lib/proportion.ts`).** The reference-IoU above is scale- and rotation-NORMALIZED (each model framed to its own bbox), so it is blind to proportion/aspect/fill/centroid errors. A SEPARATE `proportionMatch` term compares the photo mask's aspect / fill / centroid extents vs the render at one SHARED scale (the refMask 2D bbox is derived first — it is a bare `Uint8Array`). It is folded into `scoreCandidate` (best-of-N) as a tiebreak weighted BELOW the scale-blind `shapeMatch` and clamped so the combined soft tier can never cross a harder signal, and — when IoU is already below the floor — appends a specific "proportions are off" note to the refine discrepancy below `PROPORTION_FLOOR ≈ 0.7`. The scale-blind IoU port in `silhouette.ts` is UNCHANGED (bit-identical; `bench/silhouette-live.selftest.mjs` stays green) — the proportion term is a new function, never a mutation of it.
- **Per-piece reference-IoU for KITS (OC-12, `src/lib/kitScore.ts`).** For a multi-part image kit (a `part` enum whose first option is `all`), the whole-render IoU averages a single wrong piece away. After the whole-render check, each non-`all` `part` option is compiled (Draft, bounded budget) and scored against the reference mask; the WORST piece below the floor OVERRIDES the discrepancy with a piece-SPECIFIC string (names the piece + its measured overlap) so the next refine fixes that piece by name and leaves the rest. The single-part path (OC-2) is byte-identical when not a kit (gated on `isMultiPart`).
- **Live vision-judge oracle (OC-6, `src/lib/visionJudge.ts` → `POST /api/vision-judge`).** OPT-IN (off by default; `useUi.visionJudge`) and only on a vision-capable engine for an image turn. Captures the same canonical poses and POSTs them to a server route that runs the bench's `judgeVision` per-feature present/faithful + asymmetryPreserved check (`bench/judge.mjs runVisionJudge`, env-ungated core). When a NAMED feature is judged ABSENT, a bounded refine is armed citing it, through the EXISTING `refineDiscrepancy`/`pendingAutoRefineFor` seam (ChatPanel is not touched). ADVISORY ONLY — never blocks export / the green verdict; the VLM call is metered like a generation call. Off → never reached → the live path is byte-identical to today.
- Appears in chat as a `REFINE PASS` action chip with the snapshots.
- **Auto-fires** after the first image-grounded model renders (vision engine, non-local, auto-repair on) and re-arms after each refine result; the user can still trigger more manually. **Interactive auto-refine defaults to ONE pass (LAT-2):** a 2nd auto pass is armed only when the reference-IoU oracle is positively still improving; otherwise the chain stops and the user fires one more via the **"Refine again"** control (the manual refine button relabels to "Refine again" once an auto pass has run). `MAX_AUTO_REFINE` remains the hard ceiling for auto + manual combined. **Proxy-gated convergence:** when the model read off stated dimensions, the auto-refine arms ONLY while the model-independent `dimDiscrepancies` check still flags a mismatch; with no stated dims and no IoU signal, the self-relative convergence guard governs.
- **Cumulative per-turn wall-clock budget (LAT-2, `TURN_WALLCLOCK_MS ≈ 90s`):** anchored at the user-initiated send (and re-anchored on a manual "Refine again", never on an auto "Refine pass"), it bounds the whole gen→refine→autofix chain. Once exceeded, no further auto-refine or quality auto-fix pass is armed and the chain stops cleanly. A hard compile-error auto-fix is exempt (a non-compiling part must still be repaired).
...
- **Re-roll (`Regenerate`)**: a `REGENERATE · a different take · keeps this version` chip under the current model (`rerollLast`, shown only on the current code-bearing version when idle) generates a fresh version of the same request, **appended after the current one** via a `Regenerate` marker user turn — it does NOT replace or discard the current version. Both alternatives stay in the lineage as numbered versions and remain switchable via their chips (Restore / Bring-them-back), so the user picks between siblings. Like a diverging send it clears any set-aside redo tail (a re-roll is a new branch).
...
- **Per-part print quantities**: each piece chip in the PARTS bar carries an editable `×N` stepper (shown on the *selected* piece; a read-only `×N` badge marks the others when N>1). Quantity is **project metadata** (`Project.partQuantities`, keyed by the part-enum option string, absent ⇒ 1, clamped `[1,99]`) — deliberately **not** a Customizer param (a print count is not geometry, so it never recompiles or pollutes the sliders) and **not** version-scoped (untouched by rollback — a print order, not geometry). Semantics are **hybrid by export format**: where the file is a plate/scene — `Plates as .3mf`, plain `.3mf`, **and the live Slicer view** — N **replicates geometry** (each distinct piece is compiled once, then cloned N times; replicas pack as N distinct placements via unique keys, and read as **one color, N instances**). Where the file is a bare mesh — `Parts as separate .stl`, single `.stl` — N is a **count annotation only** (a note "set N copies in your slicer"), never N identical files. The live Slicer view feeds the SAME qty-expanded list into the packer as the `.3mf` export, so the on-screen plate count is WYSIWYG with the file. The `.vibemesh` share file carries `partQuantities` (schema v2, tolerant-parsed) so a remix preserves the counts. Single-object designs (no `part` enum) have no quantity affordance — set instances in the slicer.
...
- **Image/trace/facet/refine prompt layer is image-gated (RET-5).** The trace-the-defining-outline, hard-surface/faceted styling, reading-reference-images, and refine-pass blocks live in `IMAGE_PROMPT` (`server/prompt.mjs`), appended by `contextText` (`server/providers.mjs`) ONLY for an image/reference turn — detected by an attached image block in the messages (`hasImage` / `messagesHaveImage`), a carried `intent.sourceType`, or the coarse first-turn `sourceHint`. A pure-text request (a gear, a bracket) no longer carries this machinery, shrinking the always-on `SYSTEM_PROMPT` core (`SYSTEM_PROMPT_TOKENS`) by ~37% (~10.4k → ~6.5k tok). The always-on core keeps the response contract, printability/safety rules, multi-part/connector rules, geometric-consistency, and the final self-check (with a compact image-only self-check addendum that ships in `IMAGE_PROMPT`). Model-facing behavior on an image turn is unchanged — the model still receives `SYSTEM_PROMPT + IMAGE_PROMPT`.
- **Axisymmetry decision before `rotate_extrude` (OC-11).** The trace idioms in `IMAGE_PROMPT` require an explicit **axisymmetry check** before any revolve channel is chosen: a `rotate_extrude` is correct ONLY when the reference looks the same from every rotation around its central axis (chess pawns, wine glasses, round finials, hemispheres). A body whose top-view footprint is non-circular (oval outlet funnel, flat-back pod, clamshell wider than deep) is NOT axisymmetric and must NOT be forced into a revolve — the circular section collapses the proportions visible in the reference. For non-axisymmetric bodies the model uses a **hull/loft-of-two-sections** idiom: trace the FRONT section `[x,z]` and SIDE section `[x,z]` each as a single-line constant below the parameter block, then `hull()` two `linear_extrude(0.01)` of them (the side one rotated 90°); a `section_aspect` parameter (ratio of side depth to front width, `// [0.3:0.05:1.5]`) lets the user correct the proportion without re-tracing. When both sections are nearly aspect-scaled versions of each other, only the front profile need be traced and the side is derived via `scale([section_aspect,1,1])`. The image self-check in `IMAGE_PROMPT` includes an AXISYMMETRY CHECK item that confirms revolve was used only on a genuinely axisymmetric body.
- **Canonical faceting policy (RET-6).** The `$fn`/smooth-vs-faceted contract is stated ONCE as a `FACETING POLICY` block at the top of the geometry rules in `SYSTEM_PROMPT`; every later mention (the curve-resolution rule, the final-self-check item, and the IMAGE_PROMPT trace/facet/self-check blocks) REFERENCES it instead of re-deriving. The resolved contract: **smooth by default** (no global `$fn`, no `$fn`/segments/resolution parameter); a per-call `$fn` is correct ONLY for discrete-count geometry (hex socket) or under a GENUINELY-FACETED image FACET VERDICT (a `facets` param on the stylized mass). **A text-only or ambiguous request has no FACET VERDICT and stays smooth** — the prior contradiction (smooth-default stated twice vs. a facet verdict only computed for stylized refs) is gone. The SMOOTH-MACHINED greeble menu is gated behind the SCOPE GATE exactly once.
- **Figurative subjects default to the stylized-relief floor (OC-8).** For a figurative TEXT subject (chess piece, animal, figure), the DEFAULT path in `SYSTEM_PROMPT` is now a compiling stylized RELIEF (a `linear_extrude` of a clean silhouette polygon on a flat base with differenced recesses), not fragile traced 3D — full side-profile geometry is reserved for the confident case, and `minkowski()` on a figurative body is discouraged (slow CGAL fallback; use an `offset(r)offset(-r)` concavity-preserving fillet instead). On a compile failure whose error reads figurative/trace-shaped, `buildAutoFixPrompt` (`src/lib/compileReport.ts`) nudges the auto-fix turn toward that relief floor.
- **Static dead-module / phantom-param check (OC-5).** `structuralReport` (`src/lib/compileReport.ts`) additionally flags any `module NAME(` that is never called and any declared Customizer parameter referenced nowhere but its own assignment. These advisory issues feed the existing skillNote and the bounded auto-fix. Guards against false positives: per-call `$fn`, genuinely-called module-scoped helpers (counted across the whole program, so a helper called only from inside another module is fine), `_`-prefixed hidden probe knobs, `$`-vars, params consumed only by a derived value, and the `part` enum + its dispatched pieces. `SYSTEM_PROMPT` carries the matching rule: no dead modules, no leftover/invalid code, every declared parameter must affect geometry.
...
- **Advisory structuralReport checks.** Duplicate shared-concept parameters (≥2 clearance/wall) and a
  ...
  never blocking.
- **Reference-free connectivity oracle (OC-1, `islandCount` in `src/lib/stl.ts`).** After a SINGLE-solid
  part compiles, the just-rendered STL is run through a pure, three.js-free connected-components count
  (union-find over triangles sharing a ~1e-3 mm-quantized vertex; signed-volume telescoped per island
  for the largest-volume fraction). A part that renders as ≥2 disjoint islands with a non-trivial
  secondary (largest island < ~95% of mesh volume) — a handle floating off a mug wall, a detached lid —
  is physically broken despite a sane bbox and a clean compile, so it pushes an assembly issue ("fuse
  the detached feature") into the SAME bounded auto-fix turn as the structural checks (gated on
  `canRepair` + `MAX_GEOM_FIX`). A hollow ring/tube is ONE connected shell → never flagged. The
  assembled multi-part `all` view is NEVER island-checked (its pieces are legitimately separate).
